...
public class WaveDanceMLView: MTKView {
    ...
    private var pcmPushFrequency: Float = 0.1
    private var frequencyCount: Float = 0
    private var viewAllCount = 5
    ...
    private func setupGPUBuffers() {
        ...
        // pcm数据缓存区
        let bufferSize = MemoryLayout<Float64>.stride * 1024
    ...
    public func pushPCMtoGPU(buffer: AVAudioPCMBuffer, count: Int) {
        ...
        frequencyCount += 1
        if frequencyCount < pcmPushFrequency * 44.0 {
            return
        } else {
            frequencyCount = 0
        }
    ...
    private enum AnimationProsBuffer {
        static let arrayCount = 1024
